...
Write JSON unit ru.ibsqa.chameleon.tests in less code.  Great for testing REST interfaces.

Summary
-------

Write JSON ru.ibsqa.chameleon.tests as if you are comparing a string.  Under the covers, JSONassert converts your string into a JSON object and compares the logical structure and data with the actual JSON.  When _strict_ is set to false (recommended), it forgives reordering data and extending results (as long as all the expected elements are there), making ru.ibsqa.chameleon.tests less brittle.
...
if ("Carter Page".equals(friend1Obj.getString("name"))) {
    assertEquals(123, friend1Obj.getInt("id"));
    assertEquals("Corby Page", friend2Obj.getString("name"));
    assertEquals(456, friend2Obj.getInt("id"));
}
else if ("Corby Page".equals(friend1Obj.getString("name"))) {
    assertEquals(456, friend1Obj.getInt("id"));
    assertEquals("Carter Page", friend2Obj.getString("name"));
    assertEquals(123, friend2Obj.getInt("id"));
}
else {
    fail("Expected either Carter or Corby, Got: " + friend1Obj.getString("name"));
}
</del></code></pre>
...
Write ru.ibsqa.chameleon.tests like this:
