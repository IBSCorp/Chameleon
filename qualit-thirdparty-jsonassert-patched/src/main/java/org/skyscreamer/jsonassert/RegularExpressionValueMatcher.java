...
public class RegularExpressionValueMatcher<T> implements ValueMatcher<T> {
	// LOCALIZE:
	private static ILocaleManager localeManager = SpringUtils.getBean(ILocaleManager.class);
	...
	public RegularExpressionValueMatcher(String pattern) throws IllegalArgumentException {
		try {
			...
			throw new IllegalArgumentException(localeManager.getMessage("jsonassert.constant.expected.pattern.invalid", e.getMessage()), e);
			/* LOCALIZE:
	...
	public boolean equal(T actual, T expected) {
		...
		try {
			...
			if (!pattern.matcher(actualString).matches()) {
				throw new ValueMatcherException(localeManager.getMessage("jsonassert.expected.pattern.did.not.match.value", getPatternType()), pattern.toString(), actualString);
				/* LOCALIZE:
			...
			throw new ValueMatcherException(localeManager.getMessage("jsonassert.expected.pattern.invalid", getPatternType(), e.getMessage()), e, expectedString, actualString);
			/* LOCALIZE:
	...
	private String getPatternType() {
		return isStaticPattern()? localeManager.getMessage("jsonassert.constant") : localeManager.getMessage("jsonassert.dynamic");
		/* LOCALIZE:
